    ...
    while True:
        for i, ci in enumerate(clauze):
            for j, cj in enumerate(clauze):
                ...
                if set() in rezolvate:
                    return False  # Am găsit clauza vidă, deci nesatisfiabil
                noi.update(rezolvate)
        if noi.issubset(set(map(frozenset, clauze))):
            return True  # Nu s-a mai adăugat nimic nou
        clauze.extend(map(set, noi))
...
if __name__ == "__main__":
    ...
    clauze = citeste_cnf(formula)

    print("Alege metoda: 1. DPLL  2. DP  3. Rezoluție")
    metoda = input("Număr metodă: ")

    if metoda == '1':
        satisfiabila, _ = dpll(clauze)
        if satisfiabila:
            print("Formula este Satisfiabilă (DPLL).")
        else:
            print("Formula este Nesatisfiabilă (DPLL).")
    elif metoda == '2':
        satisfiabila = dp(clauze)
        if satisfiabila:
            print("Formula este Satisfiabilă (DP).")
        else:
            print("Formula este Nesatisfiabilă (DP).")
    elif metoda == '3':
        satisfiabila = rezolutie(clauze)
        if satisfiabila:
            print("Formula este Satisfiabilă (Rezoluție).")
        else:
            print("Formula este Nesatisfiabilă (Rezoluție).")
    else:
